    ...
    <div className="storefront min-h-screen bg-muted text-foreground">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        {!loading && nextService && vehicle && (
          <Card className="mb-8 rounded-2xl border-blue-200 bg-blue-50 shadow-sm">
            <CardContent className="px-6 py-5">
              <p className="text-xs font-semibold text-blue-600 uppercase tracking-wide mb-2">
                Próximo Mantenimiento
              </p>
              <h2 className="text-xl font-bold text-gray-900">{nextService.description}</h2>
        ...
        {!loading && calendar.length > 0 && vehicle && (
          <div>
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Próximos Mantenimientos</h2>
            <div className="space-y-4">
              {calendar.map((item) => (
                <CalendarItem
                  key={item.planId}

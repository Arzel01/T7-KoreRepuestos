...
planificados, hotfixes controlados y revisión obligatoria por PR. Esta guía
es la fuente de verdad para todo el equipo de Kore Repuestos.
...
- **Nadie** hace push directo a `main` ni a `develop`. Ambas exigen Pull Request.
...
4. **Squash merge** para `feature/*` y `bugfix/*` (historial limpio).
   **Merge commit** para `release/*` y `hotfix/*` (preserva trazabilidad).
...
Cada release se etiqueta automáticamente al mergear `release/*` o `hotfix/*` a `main`.
